...
The constructor of `Shopware\Core\Checkout\Cart\Price\Struct\RegulationPrice` is private, matching `ListPrice`. Replace direct instantiations with the factory, which also computes `discount` and `percentage`:
